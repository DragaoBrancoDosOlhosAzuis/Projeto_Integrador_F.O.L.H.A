...
async function carregarVendasRecentes() {
  try {
    ...
    const tbody = document.querySelector("#vendasRecentes tbody");

    if (!vendas || vendas.length === 0) {
      tbody.innerHTML = `
                <tr><td colspan="5" class="text-center">Nenhuma venda encontrada</td></tr>
            `;
      return;
    }

    // Pegue apenas as 5 mais recentes
    ...
    recentes.forEach((venda) => {
      ...
      tr.innerHTML = `
                <td>#${String(venda.id).padStart(5, "0")}</td>
                <td>${venda.data}</td>
...
function atualizarSelectProdutos() {
  ...
  if (selectProduto && produtosDisponiveis.length > 0) {
    ...
    produtosDisponiveis.forEach((produto) => {
      ...
      option.textContent = `${produto.nome} - ${Utils.formatarMoeda(
        produto.valor
      )}`;
...
function selecionarProdutoVenda(index, produtoId) {
  ...
  if (produto) {
    ...
    const itemElement = document.querySelector(
      `.produto-item-venda[data-index="${index}"]`
    );
    itemElement.querySelector(".valor-unitario").value = Utils.formatarMoeda(
      produto.valor
    );
...
function atualizarSubtotalVenda(index) {
  ...
  const quantidade =
    parseInt(itemElement.querySelector(".quantidade").value) || 0;

  produtosVenda[index].quantidade = quantidade;
  produtosVenda[index].subtotal =
    produtosVenda[index].valor_unitario * quantidade;

  itemElement.querySelector(".subtotal").value = Utils.formatarMoeda(
    produtosVenda[index].subtotal
  );
  ...
  console.log(
    `Subtotal atualizado: ${Utils.formatarMoeda(produtosVenda[index].subtotal)}`
  );
}
...
function removerProdutoVenda(index) {
  ...
  document.querySelectorAll(".produto-item-venda").forEach((item, newIndex) => {
    ...
    item
      ...
      .setAttribute(
        "onchange",
        `selecionarProdutoVenda(${newIndex}, this.value)`
      );
    item
      .querySelector(".quantidade")
      .setAttribute("onchange", `atualizarSubtotalVenda(${newIndex})`);
    item
      .querySelector(".quantidade")
      .setAttribute("oninput", `atualizarSubtotalVenda(${newIndex})`);
    item
      .querySelector("button")
      .setAttribute("onclick", `removerProdutoVenda(${newIndex})`);
  });
...
async function confirmarVenda() {
  try {
    ...
    const produtosInvalidos = produtosVenda.filter(
...
async function filtrarVendas() {
  try {
    ...
    console.log(
      `Vendas filtradas: ${vendasFiltradas.length} de ${vendas.length}`
    );
  } catch (error) {
            ...
            ">
                ...
                ">
                    <div class="modal-header" style="
                        ...
                        padding-bottom: 1rem;
                    ">
                        <h3 style="margin: 0;">Detalhes da Venda #${venda.id
                          .toString()
                          .padStart(5, "0")}</h3>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="info-row" style="display: flex; justify-content: space-between; margin-bottom: 1.5rem;">
                            <div>
                                <strong>Pagamento:</strong> ${formatarMetodoPagamento(
                            ...
                            <table style="width: 100%; border-collapse: collapse;">
                                ...
                                <tfoot>
                                    <tr style="background: #f8f9fa;">
                                        ...
                                        <td style="padding: 0.75rem; text-align: right; font-weight: bold;">${Utils.formatarMoeda(
                                          venda.total
                                        )}</td>
                                    </tr>
                                </tfoot>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(() => {
    if (
      ...
      document.getElementById("vendas").classList.contains("active")
    ) {
      inicializarVendas();
    }
  }, 100);
