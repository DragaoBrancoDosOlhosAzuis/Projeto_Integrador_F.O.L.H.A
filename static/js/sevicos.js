...
function selecionarProdutoServico(index, produtoId) {
    ...
    if (produto) {
        ...
        const itemElement = document.querySelector(`.produto-item-servico[data-index="${index}"]`);
        itemElement.querySelector('.valor-unitario-servico').value = Utils.formatarMoeda(produto.valor);
...
function atualizarSubtotalServico(index) {
    const itemElement = document.querySelector(`.produto-item-servico[data-index="${index}"]`);
    const quantidade = parseInt(itemElement.querySelector('.quantidade-servico').value) || 0;
    
    produtosServico[index].quantidade = quantidade;
    produtosServico[index].subtotal = produtosServico[index].valor_unitario * quantidade;
    
    itemElement.querySelector('.subtotal-servico').value = Utils.formatarMoeda(produtosServico[index].subtotal);
    
    calcularTotalServico();
    
    console.log(`Subtotal do serviço atualizado: ${Utils.formatarMoeda(produtosServico[index].subtotal)}`);
}
...
function removerProdutoServico(index) {
    ...
    const elemento = document.querySelector(`.produto-item-servico[data-index="${index}"]`);
    if (elemento) {
        elemento.remove();
    }
    
    // Reindexar elementos restantes
    document.querySelectorAll('.produto-item-servico').forEach((item, newIndex) => {
        item.setAttribute('data-index', newIndex);
        item.querySelector('.produto-select-servico').setAttribute('onchange', `selecionarProdutoServico(${newIndex}, this.value)`);
        item.querySelector('.quantidade-servico').setAttribute('onchange', `atualizarSubtotalServico(${newIndex})`);
        item.querySelector('.quantidade-servico').setAttribute('oninput', `atualizarSubtotalServico(${newIndex})`);
        item.querySelector('button').setAttribute('onclick', `removerProdutoServico(${newIndex})`);
    });
    
    calcularTotalServico();
    
    console.log('Produto removido do serviço. Total restante:', produtosServico.length);
}

// Calcular total do serviço
function calcularTotalServico() {
    const total = produtosServico.reduce((sum, item) => sum + item.subtotal, 0);
    document.getElementById('total-fiado').textContent = `Total: ${Utils.formatarMoeda(total)}`;
...
async function confirmarVendaFiada() {
    try {
        ...
        console.log('Dados do serviço:', { lojaId, dataServico, funcionarioNome, produtos: produtosServico });
        ...
        const produtosInvalidos = produtosServico.filter(item => !item.produto_id || item.quantidade <= 0);
...
async function obterOuCriarFuncionario(nomeFuncionario) {
    try {
        ...
        const funcionarioExistente = funcionarios.find(f => 
            f.nome.toLowerCase() === nomeFuncionario.toLowerCase()
        );
...
function limparServico() {
    ...
    document.getElementById('data-servico').value = new Date().toISOString().split('T')[0];
...
async function carregarContasReceber() {
    try {
        ...
        lojas.forEach(loja => {
            ...
            const totalFiado = servicosLoja.reduce((total, servico) => total + servico.total, 0);
            
            // Encontrar última venda
            const ultimaVenda = servicosLoja.length > 0 
                ? servicosLoja.reduce((maisRecente, servico) => {
                    return new Date(servico.data) > new Date(maisRecente.data) ? servico : maisRecente;
                })
                : null;
            
            const status = totalFiado > loja.limite_credito ? 'Limite excedido' : 'Em dia';
            const statusClass = totalFiado > loja.limite_credito ? 'pending' : 'completed';
            
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${loja.nome}</td>
                <td>${Utils.formatarMoeda(totalFiado)}</td>
                <td>${ultimaVenda ? formatarDataParaBr(ultimaVenda.data) : 'Nenhuma'}</td>
                <td>
                    <button class="btn btn-primary btn-sm" onclick="verDetalhesLoja(${loja.id})">
        ...
        if (lojas.length === 0) {
            tabela.innerHTML = '<tr><td colspan="4" class="text-center">Nenhuma loja cadastrada</td></tr>';
        }
                    ...
                    <div class="modal-body">
                        ...
                        ">
                            ...
                            ">
                                ...
                                <div style="font-size: 1.5rem; font-weight: bold;">${Utils.formatarMoeda(loja.limite_credito)}</div>
                            </div>
                            <div class="info-card" style="
                                background: #f8f9fa;
                                padding: 1rem;
                            ...
                            ">
                                ...
                                <div style="font-size: 1.5rem; font-weight: bold; color: ${totalFiado > loja.limite_credito ? '#dc3545' : '#28a745'}">${Utils.formatarMoeda(totalFiado)}</div>
                            </div>
                        </div>
                        ...
                        <div class="table-container">
                            <table style="width: 100%; border-collapse: collapse;">
                                ...
                                <tbody>
                                    ${servicosLoja.length > 0 ? servicosLoja.map(servico => `
                                        <tr>
                                            <td style="padding: 0.75rem; border-bottom: 1px solid #eee;">${servico.data}</td>
                                            <td style="padding: 0.75rem; border-bottom: 1px solid #eee;">${servico.funcionario_nome}</td>
                                            <td style="padding: 0.75rem; text-align: right; border-bottom: 1px solid #eee;">${Utils.formatarMoeda(servico.total)}</td>
                                            <td style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #eee;">${servico.itens ? servico.itens.length : 0}</td>
                                        </tr>
                                    `).join('') : `
                                        <tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        if (document.getElementById('lojas') && document.getElementById('lojas').classList.contains('active')) {
            inicializarServicos();
...
const observerServicos = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
            if (mutation.target.id === 'lojas' && mutation.target.classList.contains('active')) {
                inicializarServicos();
                carregarContasReceber();
            }
        }
