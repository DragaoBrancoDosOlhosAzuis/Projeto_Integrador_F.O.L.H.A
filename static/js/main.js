...
function atualizarInterfaceUsuario() {
    if (usuarioLogado) {
        ...
        if (userAvatar) userAvatar.textContent = usuarioLogado.nome.substring(0, 2).toUpperCase();
        if (userName) userName.textContent = usuarioLogado.nome;
        if (userRole) userRole.textContent = usuarioLogado.nivel === 'admin' ? 'Administrador' : 'Usuário';
    }
}
    ...
    try {
        ...
        if (document.getElementById('totalVendas')) {
            document.getElementById('totalVendas').textContent = 
                Utils.formatarMoeda(resumo.vendas_hoje.total);
        }
        
        if (document.getElementById('totalFiado')) {
            document.getElementById('totalFiado').textContent = 
                Utils.formatarMoeda(resumo.servicos_hoje.total);
        }
...
async function carregarFuncionarios() {
    try {
        ...
        if (tabela) {
            ...
            funcionarios.forEach(funcionario => {
                ...
                tr.innerHTML = `
                    ...
                    <td><span class="status ${funcionario.ativo ? 'completed' : 'pending'}">${funcionario.ativo ? 'Ativo' : 'Inativo'}</span></td>
                    <td>
                        <button class="btn btn-primary btn-sm" onclick="editarFuncionario(${funcionario.id})">
                            <i class="fas fa-edit"></i> Editar
                        </button>
                        <button class="btn btn-danger btn-sm" onclick="excluirFuncionario(${funcionario.id})" style="margin-left: 0.25rem;">
        ...
        const totalVendedores = funcionarios.filter(f => f.cargo === 'Vendedor' && f.ativo).length;
        const totalAdministrativos = funcionarios.filter(f => f.cargo === 'Administrativo' && f.ativo).length;
...
async function carregarLojas() {
    try {
        ...
        if (tabela) {
            ...
            lojas.forEach(loja => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="btn btn-primary btn-sm" onclick="editarLoja(${loja.id})">
                            <i class="fas fa-edit"></i> Editar
                        </button>
                        <button class="btn btn-danger btn-sm" onclick="excluirLoja(${loja.id})" style="margin-left: 0.25rem;">
                            <i class="fas fa-trash"></i> Excluir
                        </button>
                    </td>
                `;
        ...
        // Atualizar estatísticas
        const totalFiadoGeral = servicos.reduce((total, servico) => total + servico.total, 0);
        ...
        if (document.getElementById('totalFiadoGeral')) {
            document.getElementById('totalFiadoGeral').textContent = Utils.formatarMoeda(totalFiadoGeral);
        }
        ...
        if (tabela) {
            ...
            produtos.forEach(produto => {
                ...
                tr.innerHTML = `
                    ...
                    <td><span class="status ${produto.ativo ? 'completed' : 'pending'}">${produto.ativo ? 'Ativo' : 'Inativo'}</span></td>
                    <td>
                        <button class="btn btn-primary btn-sm" onclick="editarProduto(${produto.id})">
                            <i class="fas fa-edit"></i> Editar
                        </button>
                        <button class="btn btn-danger btn-sm" onclick="excluirProduto(${produto.id})" style="margin-left: 0.25rem;">
                            <i class="fas fa-trash"></i> Excluir
                        </button>
                    </td>
                `;
...
async function editarFuncionario(id) {
    try {
        ...
        if (funcionario) {
            ...
            document.getElementById('btnSalvarFuncionario').innerHTML = '<i class="fas fa-save"></i> Atualizar Funcionário';
...
async function editarProduto(id) {
    try {
        ...
        if (produto) {
            ...
            document.getElementById('btnSalvarProduto').innerHTML = '<i class="fas fa-save"></i> Atualizar Produto';
